...
# PURPOSE: Show "Expense by Category" as a table with refresh, sorting, and inclusive date range.
...
class ReportCategoryTab(ttk.Frame):
    def __init__(self, parent, dao: TransactionDAO):
        ...
        # --- Top bar: From/To pickers + Results + Total ---
        bar = ttk.Frame(self)
        ...
        self.StartPicker = DateEntry(bar, date_pattern="yyyy/mm/dd", width=12, state="readonly", style="Gill.DateEntry")
        self.StartPicker.grid(row=0, column=1, padx=(0, 12))

        ttk.Label(bar, text="To:").grid(row=0, column=2, padx=(0, 6))
        self.EndPicker = DateEntry(bar, date_pattern="yyyy/mm/dd", width=12, state="readonly", style="Gill.DateEntry")
        self.EndPicker.grid(row=0, column=3, padx=(0, 12))

        ttk.Button(bar, text="Results", style="Gill.TButton", command=self.LoadData) \
        ...
        self.Tree.heading("amount", text="Amount", command=lambda: self.SortBy("amount", False), anchor="e")
        ...
        self.LoadData()
        self.after(0, self._StabilizePickers)

    def LoadData(self):
        # Read dates from pickers and ensure start <= end
        try:
            start = self.StartPicker.get_date().strftime("%Y/%m/%d")
            end = self.EndPicker.get_date().strftime("%Y/%m/%d")
        except Exception as ex:
            messagebox.showerror("Invalid Date", f"Please choose valid dates.\n{ex}")
            return
        ...
        for c in self.Columns:
            if c not in df.columns:
                messagebox.showerror("Schema Error", "Report data missing expected columns.")
                return
        ...
        # Aggregated summary amount (inclusive range)
    ...
    def _StabilizePickers(self):
        """
        Run once after the widgets are realized. On some Tk builds this prevents the
        DateEntry popup from closing when clicking month arrows by letting the
        internal widgets fully settle.
        """
        try:
            self.update_idletasks()
            for w in (self.StartPicker, self.EndPicker):
                # Toggle state briefly to force an internal rebuild
